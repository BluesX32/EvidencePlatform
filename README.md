...
| **Citation sourcing** | After extraction, automatically fetch reference lists (backward) and citing papers (forward) for every paper in your Extraction Library via the Semantic Scholar API; **manual file import** (RIS/MEDLINE) for databases not covered by Semantic Scholar, tagged with direction and source paper; resolves papers via DOI (with `dx.doi.org` normalisation), PMID (regex extraction for annotated formats), or title+year search (±2 yr window, ±5 yr fallback); pagination driven by S2 `next` cursor; 429 rate-limit retry (30 s → 60 s → 120 s); select papers with checkboxes (select-all, server-side); papers already in the project are flagged and their status auto-updates when a corpus is deleted; import batches named after the source article so provenance is visible in the Extraction Library; iterative snowballing via All / New / Custom / Manual scope options |
