...
_ARTICLES_SQL = text(
    ...
    WITH labeled_records AS (
        ...
        WHERE rl.project_id = :project_id
          AND (CAST(:label_id AS text) IS NULL OR rl.label_id::text = CAST(:label_id AS text))
    ...
    enriched AS (
        ...
        WHERE lr.record_id IS NOT NULL
          AND (CAST(:record_id AS text) IS NULL OR lr.record_id::text = CAST(:record_id AS text))
          AND (CAST(:cluster_id AS text) IS NULL OR FALSE)
        ...
        WHERE lr.cluster_id IS NOT NULL
          AND (CAST(:cluster_id AS text) IS NULL OR lr.cluster_id::text = CAST(:cluster_id AS text))
          AND (CAST(:record_id AS text) IS NULL OR FALSE)
    )
    SELECT *
    FROM enriched
...
_ARTICLES_COUNT_SQL = text(
    ...
    WHERE rl.project_id = :project_id
      AND (CAST(:label_id AS text)   IS NULL OR rl.label_id::text   = CAST(:label_id AS text))
      AND (CAST(:record_id AS text)  IS NULL OR rl.record_id::text  = CAST(:record_id AS text))
      AND (CAST(:cluster_id AS text) IS NULL OR rl.cluster_id::text = CAST(:cluster_id AS text))
    """
)
