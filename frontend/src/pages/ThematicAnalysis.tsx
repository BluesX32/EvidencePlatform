          ...
          <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
            ...
            {onAiAssign && (
              <button
                ...
                onClick={() => onAiAssign(code.id)}
                title="Auto-assign this code to relevant papers using AI"
              >
                {aiAssigning ? "✦ Assigning…" : "✦ Auto-assign"}
              </button>
            )}
            <button
              className="btn-ghost"
...
export default function ThematicAnalysis() {
  ...
  const [suggestModel, setSuggestModel] = useState("anthropic/claude-haiku-4-5");
  const [suggestFocus, setSuggestFocus] = useState("");
  const [suggestedThemes, setSuggestedThemes] = useState<{ name: string; description: string; rationale: string }[]>([]);
  const [showSuggestPanel, setShowSuggestPanel] = useState(false);
  const [creatingThemeIdx, setCreatingThemeIdx] = useState<number | null>(null);
  const [aiAssignCodeId, setAiAssignCodeId] = useState<string | null>(null);
  const [aiAssignResult, setAiAssignResult] = useState<{ message: string } | null>(null);
  ...
  const createFromSuggestionMut = useMutation({
    mutationFn: ({ name, description, idx }: { name: string; description: string; idx: number }) => {
      setCreatingThemeIdx(idx);
      return thematicApi.createTheme(projectId!, { name, description });
    },
    onSuccess: (_, { idx }) => {
      setCreatingThemeIdx(null);
      setSuggestedThemes(prev => prev.filter((_, i) => i !== idx));
      qc.invalidateQueries({ queryKey: ["thematic", projectId] });
    },
  });

  const aiAssignMut = useMutation({
    mutationFn: (codeId: string) => {
      setAiAssignCodeId(codeId);
      setAiAssignResult(null);
      return aiPilotApi.aiAssignCode(projectId!, codeId, { model: suggestModel });
    },
    onSuccess: (r) => {
      setAiAssignResult({ message: r.data.message });
      qc.invalidateQueries({ queryKey: ["thematic", projectId] });
      qc.invalidateQueries({ queryKey: ["code-evidence", projectId, aiAssignCodeId] });
    },
    onSettled: () => setAiAssignCodeId(null),
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button
            ...
            style={{
              ...
              background: showSuggestPanel ? "#ede9fe" : "linear-gradient(135deg,#6366f1,#8b5cf6)",
              color: showSuggestPanel ? "#6d28d9" : "#fff",
      ...
      {/* AI Suggest Themes panel */}
      {showSuggestPanel && (
        <div style={{ padding: "1rem 1.25rem", background: "#faf5ff", borderBottom: "1px solid #ede9fe" }}>
          <div style={{ fontWeight: 700, fontSize: "0.85rem", color: "#6d28d9", marginBottom: "0.6rem" }}>✦ AI Theme Suggestion</div>
          <div style={{ display: "flex", gap: "0.5rem", alignItems: "flex-end", flexWrap: "wrap" }}>
            <div>
              <label style={{ fontSize: "0.73rem", fontWeight: 600, color: "#6b7280", display: "block", marginBottom: 2 }}>Model</label>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              <label style={{ fontSize: "0.73rem", fontWeight: 600, color: "#6b7280", display: "block", marginBottom: 2 }}>Focus question (optional)</label>
              <input className="form-input" value={suggestFocus} onChange={e => setSuggestFocus(e.target.value)}
                placeholder="e.g. What are barriers to implementation?" style={{ width: "100%", fontSize: "0.82rem" }} />
            </div>
            <button
              onClick={() => suggestThemesMut.mutate()}
              disabled={suggestThemesMut.isPending}
              style={{
          ...
          {suggestedThemes.length > 0 && (
            <div style={{ marginTop: "0.75rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" }}>Suggested Themes — click to create</div>
              {suggestedThemes.map((t, i) => (
                <div key={i} style={{
                  display: "flex", alignItems: "flex-start", gap: "0.6rem",
                  padding: "0.6rem 0.75rem", background: "#fff", borderRadius: "0.4rem", border: "1px solid #e5e7eb",
                }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: "0.85rem", color: "#1e293b" }}>{t.name}</div>
                    <div style={{ fontSize: "0.78rem", color: "#64748b", marginTop: 2 }}>{t.description}</div>
                  </div>
                  <button
                    disabled={creatingThemeIdx === i}
                    onClick={() => createFromSuggestionMut.mutate({ name: t.name, description: t.description, idx: i })}
                    style={{ padding: "0.25rem 0.65rem", borderRadius: "0.25rem", border: "1px solid #7c3aed", background: "#faf5ff", color: "#7c3aed", fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}
                  >{creatingThemeIdx === i ? "Creating…" : "Create →"}</button>
                </div>
              ))}
            </div>
          )}
          {aiAssignResult && <p style={{ color: "#16a34a", fontSize: "0.8rem", marginTop: "0.5rem" }}>✓ {aiAssignResult.message}</p>}
        </div>
      )}
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {selectedEntry ? (
            <CodeDetail
              ...
              onAiAssign={(codeId) => aiAssignMut.mutate(codeId)}
              aiAssigning={aiAssignMut.isPending && aiAssignCodeId === selectedEntry.code.id}
            />
          ) : (
            <div
              style={{
